...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName('view')
        .setDescription('Ver la lista de pistas en la cola')
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName('remove')
    ...
    switch (subcommand) {
            ...
            .addFields([
              {
                ...
                value: `\`${queueTracks.length} pistas\``,
                inline: true,
              },
              {
                name: '⌛ Duración Total',
      ...
      case 'remove': {
        ...
        if (player.queue.tracks?.length < removePos) {
          return interaction.reply({
            content: "❌ ¡No se puede eliminar una pista que no está en la cola!",
            ephemeral: true,
          });
        }
        ...
        const removedEmbed = new EmbedBuilder()
          ...
          .setDescription(
            `Se eliminó [${removeTrack.info.title}](${removeTrack.info.uri})`
          )
          .setThumbnail(removeTrack.info.artworkUrl)
          .addFields({
            name: '🎵 Longitud de la Cola',
            value: `\`${player.queue.tracks.length} pistas restantes\``,
            inline: true,
          })
          .setFooter({
      ...
      case 'clear': {
        ...
        const clearEmbed = new EmbedBuilder()
          ...
          .setDescription(
            `Se limpiaron con éxito \`${queueLength}\` pistas de la cola`
          )
          .setFooter({
            text: `Limpiada por ${interaction.user.tag}`,
            iconURL: interaction.user.displayAvatarURL(),
          })
          .setTimestamp();
